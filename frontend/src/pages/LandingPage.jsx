...
export default function LandingPage() {
  const features = [
    ...
    { icon: FileText, title: 'Налоги РК', text: 'Подготовка форм 200 / 220 и расчёты(В разработке).' },
    ...
    <div className='min-h-screen bg-slate-950 text-white overflow-hidden'>
        ...
        <nav className='hidden md:flex gap-8 text-slate-300'>
          ...
          <a href='#why'>Почему мы</a>
            ...
            <div className='flex items-center gap-4'>
                <Link to='/login' className='px-5 py-2 rounded-2xl bg-gray-600 hover:bg-gray-500 transition'>Войти</Link>
                <Link to='/register' className='px-5 py-2 rounded-2xl bg-blue-600 hover:bg-blue-500 transition'>Регистрация</Link>
            </div>
      </header>
      ...
      <section className='relative max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center'>
        <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <div className='flex flex-wrap gap-4 mt-8'>
            <Link to='/register' className='px-8 py-4 rounded-2xl bg-blue-600 hover:bg-blue-500 transition'>Начать бесплатно</Link>
      ...
      <section id='why' className='relative max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-10'>
        ...
        <div className='rounded-3xl bg-gradient-to-br from-blue-600 to-purple-600 p-8'>
          ...
          <Link to='https://www.youtube.com/watch?v=vvbGUFe18ig' className='px-8 py-4 rounded-2xl bg-white text-slate-900 font-semibold inline-block'>Попробовать</Link>
        </div>
      </section>
